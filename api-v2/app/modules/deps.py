...
from app.modules.access.adapters.aim import AIM
import app.modules.access.adapters.aim_cognito as aim_cognito
import app.modules.access.adapters.aim_in_memory as aim_in_memory
...
def get_aim(settings: SettingsDep, cognito_client: CognitoIdpDep):
    global _aim
    if _aim is None:
        if settings.ENV == config.Environments.PRODUCTION:
            _aim = aim_cognito.AIMCognito(settings, cognito_client)
        else:
            _aim = aim_in_memory.AIMInMemory(settings)

    return _aim


AimDep = Annotated[AIM, Depends(get_aim)]


def requires_auth(request: Request, aim: AimDep):
    ...
    result = aim.verify_claims(access_token=decoded_access_token,
