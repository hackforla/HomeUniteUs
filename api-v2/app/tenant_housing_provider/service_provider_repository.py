...
# Local
from model import HousingProgramServiceProvider
...
class HousingProviderRepository:
    ...
    def create_service_provider(
            self,
            provider_name: str) -> Optional[HousingProgramServiceProvider]:
        """
        ...
        with DataAccessLayer.session() as session:
            existing_provider = session.execute(
                select(HousingProgramServiceProvider).filter(
                    HousingProgramServiceProvider.provider_name ==
                    provider_name)).scalar_one_or_none()
    ...
    def get_service_provider_by_id(
            self, provider_id: int) -> Optional[HousingProgramServiceProvider]:
    ...
    def update_service_provider(
            self, new_name: str,
            provider_id: int) -> Optional[HousingProgramServiceProvider]:
        """
        ...
        """
        with DataAccessLayer.session() as session:
            provider_to_update = session.get(HousingProgramServiceProvider,
                                             provider_id)
    ...
    def provider_count(self, existing_session: Session = None):
        ...
        def count(lcl_session: Session):
            return lcl_session.scalar(
                select(func.count(HousingProgramServiceProvider.id)))
