...
class AWSTemporaryUserpool():
    ...
    def create(self):
        ...
        self.tmp_client_secret = client_response['UserPoolClient']['ClientSecret']

        self.cognito_client.create_group(GroupName='Admins',
                                         UserPoolId=mock_pool_id)
        self.cognito_client.create_group(GroupName='Hosts',
                                         UserPoolId=mock_pool_id)
        self.cognito_client.create_group(GroupName='Guests',
                                         UserPoolId=mock_pool_id)
        self.cognito_client.create_group(GroupName='Coordinators',
                                         UserPoolId=mock_pool_id)
...
#                     f"Failed to create test user: {email}: {e.error}")

#         self.test_users_created = True

#     def add_aws_userpool_user(self, email, password, attributes=None):
#         """Adds a new user to the temporary user pool.

#         Adds the user with the given username, password, and attributes.
...
#                 f"Added user {email} to the temporary user pool")
#             return response
...
#         if request.endpoint and ('signup' in request.endpoint.lower()
...
#         self.app.config["COGNITO_ACCESS_ID"] = self.mock_service.FAKE_KEYS[
#             'AWS_ACCESS_KEY_ID']
#         self.app.config["COGNITO_ACCESS_KEY"] = self.mock_service.FAKE_KEYS[
