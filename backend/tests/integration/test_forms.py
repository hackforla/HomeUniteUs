...
from app.repositories.forms import FormsRepository
from app.repositories.user_repo import UserRepository, UserRole
...
def test_add_get_responses(empty_db_session_provider):
    with empty_db_session_provider.session() as session:
        ...
        expected_responses = [{
            ...
            user_id,
            "field_id":
            _get_field_id(retrieved_form, 'position'),
            "answer_text":
            "Designer"
        }, {
            "user_id":
            user_id,
            "field_id":
            _get_field_id(retrieved_form, 'service_length'),
            "answer_text":
            "5"
        }, {
            "user_id":
            user_id,
            "field_id":
            _get_field_id(retrieved_form, 'start date'),
            "answer_text":
            '2024-05-19'
        }]
        form_repo.add_user_responses(user_id, expected_responses)

        retrieved_answers = form_repo.get_user_responses(
            user_id, created_form_id)
