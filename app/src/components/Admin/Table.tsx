...
const MatchTable = (props: {
    ...
    const hostQuestionsFailed = data.matchResults
        ...
        .reduce<Map<number, Array<number>>>(
            (prev: Map<number, Array<number>>, cur: MatchResult) => {
                prev.set(
                    cur.hostId,
                    cur.restrictionsFailed.map(
                        (r: Restriction) => r.hostQuestionId
                    )
                )
                return prev
            },
    ...
    const interestByHostId: InterestMapping = React.useMemo(() => {
        return data.matchResults
            .filter(
                (matchResult: MatchResult) => matchResult.guestId === guestId
            )
    ...
    const hostTypeDisplay = (t: HostHomeType) => {
        switch (t) {
            ...
            default:
                throw new Error(
                    `Unhandled host home type: ${JSON.stringify(t)}`
                )
        }
    }
                                        ...
                                        <AdminGuestStyle.TableCell key={index}>
                                            {(() => {
                                                if (
                                                    ...
                                                    if (
                                                        !hostQuestionsFailed.has(
                                                            host.id
                                                        )
                                                    ) {
                                                        return (
                                                            <SuccessCell
                                                                value={hostTypeDisplay(
                                                                    host.type
                                                                )}
                                                            />
                                                        )
                                                    ...
                                                    const isFailed =
                                                        failedQuestionsForHost.indexOf(
                                                            q.id
                                                        ) >= 0
