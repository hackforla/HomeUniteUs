...
const MatchTable = (props: {
  ...
  const hostQuestionsFailed = data.matchResults
    ...
    .reduce<Map<number, Array<number>>>(
      (prev: Map<number, Array<number>>, cur: MatchResult) => {
        prev.set(
          cur.hostId,
          cur.restrictionsFailed.map((r: Restriction) => r.hostQuestionId)
        )
        return prev
      },
  ...
  const interestByHostId: InterestMapping = React.useMemo(() => {
    return data.matchResults
      ...
      .reduce<InterestMapping>(
        (map: InterestMapping, matchResult: MatchResult, index: number) => {
  ...
  const FailCell = (props: { value: string }) => (
    <div>
      <div style={{ padding: '0px 5px', textAlign: 'center' }}>
        <FontAwesomeIcon icon={faTimesCircle} style={{ color: 'red' }} />
      </div>
      <div style={{ padding: '0px 3px', textAlign: 'center' }}>
        {props.value}
      </div>
    </div>
  )

  const SuccessCell = (props: { value: string }) => (
      ...
      <AdminGuestStyle.AdminGuestPaper>
        ...
        <AdminGuestStyle.TableHeaderRow>
          ...
          {data.hostQuestions.map((q: HostQuestion, index: number) => {
            return (
              <AdminGuestStyle.TableHeaderLabel center={true} key={index}>
                {q.displayName}
              </AdminGuestStyle.TableHeaderLabel>
            )
          })}
              ...
              <AdminGuestStyle.TableCell
                onClick={
                  props.allowClick
                    ? () => {
                        console.log(
                          `AdminGuestView:MatchTable: guestId = ${guestId}`
                        )
                        console.log(
                  ...
                  <AdminGuestStyle.TableCell key={index}>
                    {(() => {
                      if (q.questionKey === 'duration_of_stay') {
                        ...
                        const failedQuestionsForHost = hostQuestionsFailed.get(
                          host.id
                        ) as Array<number>
                        const isFailed =
                          failedQuestionsForHost.indexOf(q.id) >= 0
                      ...
                      if (q.questionKey === 'hosting_amount') {
                        if (!hostQuestionsFailed.has(host.id)) {
                          return (
                            <SuccessCell
                              value={host.hostingAmount.toString()}
                            />
                          )
                        }
                      ...
                      const response = data.hostResponses.find(
                        (hr: HostResponse) =>
                          hr.hostId == host.id && hr.questionId == q.id
                      )

                      if (!response) {
                        return 'Not answered'
                      }
                      return response.responseValues.map(
                        (rvId: number, index: number) => {
                          const rv = data.responseValues.find(
                            (rv: ResponseValue) => rv.id === rvId
                          )
                          if (!rv) {
                            throw new Error(
                              `Unknown response value ID: ${rvId}`
                            )
                          }
