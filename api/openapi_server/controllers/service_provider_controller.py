...
from openapi_server.models.database import housingProgramServiceProviderSchema, housingProgramServiceProviderListSchema
...
def create_service_provider(body: dict) -> Response:
    ...
    newProvider = housing_provider_repository.create_service_provider(provider_name)
    if newProvider:
        return housingProgramServiceProviderSchema.dump(newProvider), 201
...
def get_service_provider_by_id(provider_id: int) -> Response:
    ...
    if provider:
        return housingProgramServiceProviderSchema.dump(provider), 200
...
def get_service_providers() -> Response:
    ...
    return housingProgramServiceProviderListSchema.dump(providers), 200
...
def update_service_provider(provider_id: int, body: dict) -> Response:
    ...
    if updated_provider:
        return housingProgramServiceProviderSchema.dump(updated_provider), 200
